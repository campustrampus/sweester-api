...
BUILD_ENV ?= dev
CHECK_STACK ?= true
PREFIX_NAME ?= sweester
STACK_NAME ?= rest

GITHUB_REPO := {{sweester}}
GITHUB_CORP := sweester
...
deploy-dataUploader-dev: build
	@echo
	@echo "--- Deploying dataUploader [dev] ---"
	$(MAKE_DEPLOY) dataUploader dev
	@echo "- DONE: $@"
	@echo

deploy-dataUploader-test: build
	@echo
	@echo "--- Deploying dataUploader [test] ---"
	$(MAKE_DEPLOY) dataUploader test
	@echo "- DONE: $@"
	@echo

deploy-dataUploader-prod: build
	@echo
	@echo "--- Deploying dataUploader [prod] ---"
	$(MAKE_DEPLOY) dataUploader prod
	@echo "- DONE: $@"
	@echo

deploy-dev: deploy-dataUploader-dev
	@echo
	@echo "- DONE: $@"

deploy-prod: deploy-dataUploader-prod
	@echo
	@echo "- DONE: $@"

deploy-test: deploy-dataUploader-test
	@echo
	@echo "- DONE: $@"
...
ifeq ("$(wildcard /.dockerenv)","")
	...
	DOCKER_USER=$(DOCKER_USER) DOCKER_NAME=$(DOCKER_IMAG) \
	$(MAKE_RUN) cmd
else
...
test: venv
	  . ./$(PYVENV_NAME)/bin/activate; \
	  echo "--- Setup $(PROJECT) develop [$@] ..."; \
	  $(PYVENV_NAME)/bin/pip install -r requirements-dev.txt; \
	  echo; \
	  pip list; \
	  echo; \
	  echo "--- Starting pytest for all tests ..."; \
	  AWS_DEFAULT_REGION=$(AWS_DEFAULT_REGION); \
	 $(PYVENV_NAME)/bin/pytest -c setup.cfg $(PYTEST_ARGS); \
